...
- `/` 搜索保留裸词兼容行为，同时支持 `subject:`、`from:`、`id:`、`after:`、`before:`、
  `review:` 和 `is:patch` 字段；多个 term 按 AND 组合，前缀 `-` 表示否定，单引号/双引号
  可包裹含空格的值。
- 日期条件按邮件 `Date` 解析为 `YYYY-MM-DD`，review 条件复用 Review Inbox 聚合状态；
  非法字段、日期或枚举值会在搜索状态栏给出错误并清空匹配结果，不会悄悄退回全文搜索。
- 查询解析独立于 TUI 状态机并有单元测试，后续 Saved Views 直接保存原始查询字符串。

## 25. M14（已完成）：Saved Views

### 25.1 已决策项

- `view save NAME` 保存当前已应用的结构化查询；同名视图大小写不敏感地更新，名称只允许
  字母、数字、`-`、`_` 和 `.`，避免命令栏解析歧义。
- `view use NAME` 复用原始查询并立即刷新线程列表，`view list` 展示名称和查询，
  `view delete NAME` 删除视图；命令栏提供对应补全。
- Saved View 保存在已有 `ui-state.toml`，读取时去重、丢弃空项并按名称排序，不新增数据库
  migration，也不把派生的匹配结果持久化。

## 26. M15（已完成）：邮件 Diff/代码语义着色

### 26.1 已决策项

- Preview 保持邮件正文的纯文本内容不变，只在 TUI 渲染层将邮件头部、diff 文件标记、hunk
  标记、新增/删除行、引用上下文、代码围栏和解析警告映射为语义颜色。
- 新增行使用绿色、删除行使用红色，便于在 Linux kernel mailing list 中快速扫描 patch；
  diff 文件标记使用洋红色，hunk 标记使用蓝色，头部和代码围栏使用青色，警告使用黄色，
  引用使用浅灰色以保证在常见终端主题下的可读性。
- 滚动上限仍基于未着色的 Preview 文本计算，颜色不会改变换行、滚动或邮件存储语义；普通
  叙述保持默认终端样式，降低误把自然语言当作 patch 的风险。

### 26.2 风险与后续动作

- 当前规则针对 unified diff、常见邮件头部和 Markdown fenced code；后续可按用户反馈增加
  ANSI 终端兼容性探测、主题配置或更细粒度的代码语法高亮，但不应把渲染依赖扩散到同步和存储层。
