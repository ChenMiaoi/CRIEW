...
const GIT_SENDEMAIL_SMTP_SERVER_ARGS: &[&str] = &["config", "--get", "sendemail.smtpServer"];
const GIT_SENDEMAIL_SMTP_USER_ARGS: &[&str] = &["config", "--get", "sendemail.smtpUser"];
const GIT_SENDEMAIL_SMTP_PASS_ARGS: &[&str] = &["config", "--get", "sendemail.smtpPass"];
    ...
    F: Fn(&str) -> Option<String>,
{
    ...
    let smtp_server = git_config_optional(GIT_SENDEMAIL_SMTP_SERVER_ARGS);
    let gmail = email.as_deref().is_some_and(is_gmail_address)
        || smtp_server.as_deref().is_some_and(is_gmail_server);
    let config = ImapConfig {
        email: email.clone(),
        user: normalize_optional_string(file_config.user.clone())
            .or(email.clone())
            .or_else(|| git_config_optional(GIT_SENDEMAIL_SMTP_USER_ARGS)),
        pass: normalize_optional_string(file_config.pass.clone())
            .or_else(|| git_config_optional(GIT_SENDEMAIL_SMTP_PASS_ARGS)),
        server: normalize_optional_string(file_config.server.clone())
            .or_else(|| gmail.then(|| "imap.gmail.com".to_string())),
        server_port: file_config.serverport.or(gmail.then_some(993)),
        encryption: file_config
            .encryption
            .or(gmail.then_some(ImapEncryption::Tls)),
...
fn git_config_optional(args: &[&str]) -> Option<String> {
    let output = ProcessCommand::new("git").args(args).output().ok()?;
    if !output.status.success() {
        return None;
    }
    normalize_optional_string(Some(String::from_utf8_lossy(&output.stdout).into_owned()))
}

fn is_gmail_address(value: &str) -> bool {
    value.trim().to_ascii_lowercase().ends_with("@gmail.com")
}

fn is_gmail_server(value: &str) -> bool {
    value.trim().to_ascii_lowercase().contains("gmail.com")
}
